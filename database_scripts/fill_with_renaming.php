...
// -------------- files --------
$sports_facilities_file = file_get_contents('../data/sports_facilities.json');
$sports_facilities_json = json_decode($sports_facilities_file, true);

$olympic_events_file = file_get_contents('../data/olympic_events.json');
$olympics_json = json_decode($olympic_events_file, true);

$sports_families_file = file_get_contents('../data/sports_families.json');
$sports_families_json = json_decode($sports_families_file, true);
...
foreach($sports_facilities_json as $sports_facility) {
    ...
    if (isset($sports_facility['fields']['insnovoie'])
        ...
        address_number,
        address_street,
        arrondissement_insee_code,
        id_sports_practice,
        id_sports_facility_type) VALUES (
        :practice_level,
        ...
        $request->bindParam(':arrondissement_insee_code', $sports_facility['fields']['insarrondissement']);
...
foreach ($olympics_json as $olympic_event) {
    ...
    foreach($olympic_event['dates'] as $date) {
        $request = $pdo->prepare('INSERT INTO olympic_event(event_name, event_place, arrondissement_insee_code, date, id_sports_practice)
        VALUES (
        :olympic_event_name,
        :place,
        :arrondissement_insee_code,
        ...
        $request->bindParam(':arrondissement_insee_code', $olympic_event['arrondissementInsee']);
